...
#define MAX_PAYLOAD  64               /* keep small for demo                     */
...
{
    const char* message = "Hola STM32!";
    uint16_t msgLen = strlen(message);

    // Prepare header: two-byte address + one-byte channel
    packet[0] = 0x00;
    packet[1] = 0x09;  // Example Address
    packet[2] = 0x12;  // Example Channel

    memcpy(&packet[3], message, msgLen);

    HAL_UART_Transmit(&huart1, packet, msgLen + 3, 1000);
...
// Global packet buffer – ensure it is large enough.
uint8_t packet64[64];
...
void sendPacket2(void)
{
    ...
    // Calculate total packet length (NO RSSI byte)
    uint8_t totalLen = 1 + 2 + 1 + msgLen;  // Removed "+ 1" for RSSI

//    packet64[0] = totalLen;         // Packet length
//    packet64[1] = 0x00;             // Address high byte
//    packet64[2] = 0x09;             // Address low byte
//    packet64[3] = 0x12;             // Channel
//
//    // Copy the payload message
//    memcpy(&packet64[4], message, msgLen);

    // REMOVE THIS LINE - Don't add dummy RSSI
    // packet64[4 + msgLen] = 100;

    // Transmit the packet
    HAL_UART_Transmit(&huart1, packet64, msgLen, 1000);
...
HAL_StatusTypeDef E22900T22D_Send(UART_HandleTypeDef *huart, uint8_t *data, uint16_t len)
{
    // Transmit data via the provided UART. Using a blocking transmit here.
